...
function TreeSVG({ heap, hlIndices, compareIndices, swapIndices }) {
  if (heap.length === 0) return <div className="minheap-svg-empty">Heap is empty</div>;
  ...
  return (
    <div className="minheap-svg-scroll">
...
function ArrayPanel({ heap, hlIndices, compareIndices, swapIndices }) {
  return (
    <div className="minheap-array-panel">
      <div className="minheap-array-title">Array Representation</div>
      <div className="minheap-array-container">
        {heap.length === 0 && <div className="minheap-svg-empty" style={{padding: '10px'}}>Empty array</div>}
        {heap.map((val, i) => {
          const isSwap = swapIndices?.includes(i);
          const isCmp = compareIndices?.includes(i);
          const isHl = hlIndices?.includes(i);
          let cls = 'minheap-array-cell';
          if (isSwap) cls += ' swap';
          else if (isCmp) cls += ' compare';
          else if (isHl) cls += ' highlight';
          
          return (
            <div key={i} className="minheap-array-cell-wrap">
              <div className="minheap-array-index">{i}</div>
...
function OpPanel({ onInsert, onExtract, onClear, running }) {
  ...
  return (
    <div className="minheap-op-panel">
      <div className="minheap-tab-content">
        <div className="minheap-tab-form">
          <input type="number" value={val} onChange={e => setVal(e.target.value)} placeholder="Value to insert" disabled={running} />
          <button className="minheap-btn" onClick={() => { if(val) { onInsert(+val); setVal(''); }}} disabled={running || !val}>
            Insert & Bubble Up
          </button>
          <button className="minheap-btn" style={{ background: 'var(--bg-hover)', color: 'var(--text-primary)', border: '1px solid var(--border-dim)' }} onClick={() => onInsert(Math.floor(Math.random() * 100) + 1)} disabled={running}>
            Random Insert
          </button>
          
          <button className="minheap-btn minheap-btn-danger" onClick={onExtract} disabled={running}>
            Extract Top & Bubble Down
          </button>

          <button className="minheap-btn minheap-btn-reset" onClick={onClear} disabled={running}>
            Clear Heap
          </button>
        </div>
      </div>
...
export default function MinHeapVisualizer() {
  ...
  const [ops, setOps] = useState(['Initialized Min-Heap']);
  ...
  const compare = (a, b) => a < b; // Min-Heap logic
  ...
  const handleInsert = async (val) => {
    ...
    while (i > 0) {
      ...
      if (compare(h[i], h[p])) {
        ...
        h[i] = h[p];
        h[p] = temp;
        steps.push({ h: [...h], swap: [i, p], hl: [p], line: 7, msg: `Swapped! Child was smaller than parent.` });
        i = p;
      } else {
  ...
  const handleClear = () => { setHeap([]); setOps(['Cleared Min-Heap']); setActiveCode('default'); setActiveLine(0); };

  return (
    <div className="minheap-root">
      <div className="minheap-header">
        <div>
          <h2 className="minheap-title">⬇️ Min-Heap</h2>
          <p className="minheap-desc">Complete binary tree where parent is always smaller than its children.</p>
        </div>
        <div className="minheap-badges">
          <span className="minheap-badge">Array-backed</span>
          <span className="minheap-badge">O(log n) Insert/Extract</span>
        </div>
      </div>

      <div className="minheap-content">
        <div className="minheap-visual">
          <div className="minheap-visual-title">Tree Representation</div>
          ...
          <div className="minheap-log">
            <div className="minheap-log-label">Operation Log</div>
            <div className="minheap-log-items">
              {ops.slice(-4).map((op, idx) => <div key={idx} className="minheap-log-item">{op}</div>)}
            </div>
          </div>
        </div>

        <div className="minheap-controls">
          <OpPanel 
            onInsert={handleInsert} onExtract={handleExtract} onClear={handleClear}
            running={running} 
          />
          
          <div className="minheap-formulas">
            <div className="minheap-formulas-title">Array Index Formulas</div>
            <div className="minheap-formula-row"><span>Parent of i</span> <span className="minheap-formula-code">⌊(i - 1) / 2⌋</span></div>
            <div className="minheap-formula-row"><span>Left Child of i</span> <span className="minheap-formula-code">2i + 1</span></div>
            <div className="minheap-formula-row"><span>Right Child of i</span> <span className="minheap-formula-code">2i + 2</span></div>
          </div>
        </div>
      </div>
      
      <div className="minheap-code-wrap" style={{ marginTop: '16px', background: 'var(--bg-panel)', border: '1px solid var(--border-dim)', borderRadius: 'var(--radius-lg)', padding: '14px' }}>
        <Pseudocode code={MINHEAP_PSEUDOCODES[activeCode]} activeLine={activeLine} />
      </div>
    </div>
  );
}
